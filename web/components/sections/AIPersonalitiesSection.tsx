      ...
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed">
            Choose from 10 specialized AI prompt styles, each designed for different analysis
              ...
              {personalities.map((personality, index) => (
                ...
                >
                  <div className="flex items-start space-x-4">
                    <div
                      className={`w-12 h-12 rounded-lg bg-gradient-to-r ${personality.gradient} p-3 flex-shrink-0`}
                    >
                      {(() => {
            ...
            >
              <div className="p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10">
                <div className="flex items-center space-x-4 mb-6">
                  <div
                    className={`w-16 h-16 rounded-xl bg-gradient-to-r ${personalities[selectedPersonality].gradient} p-4`}
                  >
                    {(() => {
                    ...
                    <ul className="space-y-2">
                      {personalities[selectedPersonality].focus.map((area, index) => (
                        <li key={index} className="flex items-center text-sm text-gray-300">
                          <div
                            className={`w-1.5 h-1.5 rounded-full bg-gradient-to-r ${personalities[selectedPersonality].gradient} mr-3`}
                          />
                          {area}
                        </li>
                      ))}
                ...
                <div className="mt-6 p-4 rounded-lg bg-black/20 border border-white/10">
                  ...
                  <div className="text-xs text-gray-400 font-mono leading-relaxed">
                    🔍 {personalities[selectedPersonality].name} Analysis:
                    <br />
                    • Technical complexity: High
                    <br />
                    • Impact scope: Critical
                    <br />• Recommended action: Immediate review
                  </div>
                </div>
              </div>
            </motion.div>
