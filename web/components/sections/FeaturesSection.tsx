  ...
  const features = [
    {
      ...
      description: "OpenAI GPT integration with 10 specialized prompt styles for different analysis needs",
    ...
    },
    {
      ...
      description: "Instant webhook processing with sub-100ms response times for immediate insights",
          ...
          {features.map((feature, index) => (
            ...
            >
              <div className="h-full p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/20 transition-all duration-300 hover:bg-white/10">
                ...
                <ul className="space-y-2">
                  {feature.details.map((detail, detailIndex) => (
                    <li key={detailIndex} className="flex items-center text-sm text-gray-400">
                      <div className={`w-1.5 h-1.5 rounded-full bg-gradient-to-r ${feature.gradient} mr-3`} />
                      {detail}
                    </li>
                  ))}
                </ul>
                ...
                {/* Hover Glow Effect */}
                <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${feature.gradient} opacity-0 group-hover:opacity-10 transition-opacity duration-300 pointer-events-none`} />
              </div>
            </motion.div>
          ))}
        </div>
